...
//------------------------------------------------------------------------------
static const unsigned char vtkBase64EncodeTable[65] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                                               "abcdefghijklmnopqrstuvwxyz"
                                                               "0123456789+/";

//------------------------------------------------------------------------------
inline static unsigned char vtkBase64EncodeChar(unsigned char c)
{
  assert(c < 65);
  return vtkBase64EncodeTable[c];
}

//------------------------------------------------------------------------------
void vtkBase64::EncodeTriplet(unsigned char i0, unsigned char i1, unsigned char i2,
  unsigned char* o0, unsigned char* o1, unsigned char* o2, unsigned char* o3)
{
  *o0 = vtkBase64EncodeChar((i0 >> 2) & 0x3F);
  *o1 = vtkBase64EncodeChar(((i0 << 4) & 0x30) | ((i1 >> 4) & 0x0F));
  *o2 = vtkBase64EncodeChar(((i1 << 2) & 0x3C) | ((i2 >> 6) & 0x03));
  *o3 = vtkBase64EncodeChar(i2 & 0x3F);
}

//------------------------------------------------------------------------------
void vtkBase64::EncodePair(unsigned char i0, unsigned char i1, unsigned char* o0,
  unsigned char* o1, unsigned char* o2, unsigned char* o3)
{
  *o0 = vtkBase64EncodeChar((i0 >> 2) & 0x3F);
  *o1 = vtkBase64EncodeChar(((i0 << 4) & 0x30) | ((i1 >> 4) & 0x0F));
  *o2 = vtkBase64EncodeChar(((i1 << 2) & 0x3C));
  *o3 = '=';
}

//------------------------------------------------------------------------------
void vtkBase64::EncodeSingle(
  unsigned char i0, unsigned char* o0, unsigned char* o1, unsigned char* o2, unsigned char* o3)
{
  *o0 = vtkBase64EncodeChar((i0 >> 2) & 0x3F);
  *o1 = vtkBase64EncodeChar(((i0 << 4) & 0x30));
  *o2 = '=';
  *o3 = '=';
}

//------------------------------------------------------------------------------
unsigned long vtkBase64::Encode(
  const unsigned char* input, unsigned long length, unsigned char* output, int mark_end)
{
  ...
  if (end - ptr == 2)
  {
    vtkBase64::EncodePair(ptr[0], ptr[1], &optr[0], &optr[1], &optr[2], &optr[3]);
    optr += 4;
  }
  ...
  {
    vtkBase64::EncodeSingle(ptr[0], &optr[0], &optr[1], &optr[2], &optr[3]);
    optr += 4;
  }
...
//------------------------------------------------------------------------------
static const unsigned char vtkBase64DecodeTable[256] = {
...
//------------------------------------------------------------------------------
inline static unsigned char vtkBase64DecodeChar(unsigned char c)
{
  return vtkBase64DecodeTable[c];
}

//------------------------------------------------------------------------------
int vtkBase64::DecodeTriplet(unsigned char i0, unsigned char i1, unsigned char i2,
  unsigned char i3, unsigned char* o0, unsigned char* o1, unsigned char* o2)
{
  unsigned char d0, d1, d2, d3;

  d0 = vtkBase64DecodeChar(i0);
  d1 = vtkBase64DecodeChar(i1);
  d2 = vtkBase64DecodeChar(i2);
  d3 = vtkBase64DecodeChar(i3);
  ...
  const unsigned char* input, size_t inputLen, unsigned char* output, size_t outputLen)
{
  ...
  while (inIdx <= inputLen - 4)
  {
    ...
    int bytesDecoded = vtkBase64::DecodeTriplet(
