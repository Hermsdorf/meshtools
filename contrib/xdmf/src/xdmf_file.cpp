...
XDMFFile::XDMFFile(MPI_Comm comm, const std::filesystem::path &filename,
    ...
    : _comm(comm), _filename(filename),
      _xml_spatial_doc(new pugi::xml_document), _encoding(encoding), _compression(compression)
{
    ...
    if (_encoding == Encoding::HDF5)
    {
        ...
        _h5_id = hdf5::open_file(_comm.comm(), hdf5_filename, file_mode, mpi_io);
        assert(_h5_id > 0);
    }
    else
    {
...
void XDMFFile::write_mesh(const Mesh &mesh, std::string xpath, double time)
{
    ...
    assert(grid_node);
    grid_node.append_attribute("Name") = "Partitions";
    grid_node.append_attribute("GridType") = "Collection";
    grid_node.append_attribute("CollectionType") = "Spatial";

    ofstringstream mesh_name;
    mesh_name << "/Mesh/Part_" << std::setfill('0') << std::setw(6) << MPI::rank(_comm.comm());

    pugi::xml_node time_node = grid_node.append_child("Time");
    time_node.append_attribute("TimeType") = "Single";
    time_node.append_attribute("Value")    = time;
                    ...
                    std::string xpath) const
{
    ...
    mesh::Mesh<double> mesh = mesh::create_mesh(_comm.comm(), cells, element, _x, xshape, mode);
    mesh.name = name;
    return mesh;
}
//-----------------------------------------------------------------------------
...
XDMFFile::read_topology_data(std::string name, std::string xpath) const
{
    ...
    pugi::xml_node grid_node = node.select_node(("Grid[@Name='" + name + "']").c_str()).node();
    if (!grid_node)
                              ...
                              std::string mesh_xpath)
{
    ...
    std::string timegrid_xpath = "/Xdmf/Domain/Grid[@GridType='Collection'][@Name='" + u.name + "']";
    pugi::xml_node timegrid_node = _xml_doc->select_node(timegrid_xpath.c_str()).node();
    ...
    const std::string ref_path = "xpointer(" + mesh_xpath + "/*[self::Topology or self::Geometry])";
                        ...
                        std::string xpath)
{
    ...
    pugi::xml_node values_data_node = grid_node.child("Attribute").child("DataItem");
    const std::vector values = xdmf_utils::get_dataset<std::int32_t>(
        _comm.comm(), values_data_node, _h5_id);

    const std::pair<std::string, int> cell_type_str = xdmf_utils::get_cell_type(grid_node.child("Topology"));
    ...
    std::size_t num_vertices_per_entity = mesh::cell_num_entities(
        ...
        0);
    const graph::AdjacencyList<std::int32_t> entities_adj = graph::regular_adjacency_list(std::move(entities_values.first),
                                                        ...
                                                        std::string xpath)
{
    ...
    if (!node)
        throw std::runtime_error("XML node '" + xpath + "' not found.");
    pugi::xml_node grid_node = node.select_node(("Grid[@Name='" + grid_name + "']").c_str()).node();
    if (!grid_node)
    ...
    // Get cell type
    const std::pair<std::string, int> cell_type_str = xdmf_utils::get_cell_type(topology_node);
...
std::string XDMFFile::read_information(std::string name, std::string xpath)
{
    ...
    if (!node)
        throw std::runtime_error("XML node '" + xpath + "' not found.");
    pugi::xml_node info_node = node.select_node(("Information[@Name='" + name + "']").c_str()).node();
    if (!info_node)
