...
xdmf_utils::get_cell_type(const pugi::xml_node& topology_node)
{
  ...
  const static std::map<std::string, std::pair<std::string, int>> xdmf_to_meshtools
      = {
         {"triangle", {"TRI3", 1}},
         {"tetrahedron", {"TET4", 1}},
         {"quadrilateral", {"QUAD4", 1}},
         {"hexahedron", {"HEX8", 1}},
        };

  // Convert XDMF cell type string to MeshTools cell type string
  std::string cell_type = type_attr.as_string();
  std::transform(cell_type.begin(), cell_type.end(), cell_type.begin(),
                 [](unsigned char c) { return std::tolower(c); });
  auto it = xdmf_to_meshtools.find(cell_type);
  if (it == xdmf_to_meshtools.end())
  {
...
// //----------------------------------------------------------------------------
// std::string xdmf_utils::vtk_cell_type_str(int cell_type,
//                                           int num_nodes)
// {
//   static const std::map<int, std::map<int, std::string>> vtk_map = {
