#ifndef XDMF_UTILS_H__
#define XDMF_UTILS_H__
...
/**
 * @brief Get the cell type object
 * 
 * @param topology_node 
 * @return * Get 
 */
/// @return MeshTools cell type and polynomial degree
std::pair<std::string, int> get_cell_type(const pugi::xml_node& topology_node);

/**
 * @brief  Return (0) HDF5 filename and (1) path in HDF5 file from a DataItem
 * 
 * @param dataitem_node 
...
/**
 * @brief Get the hdf5 filename object
 * 
 * @param xdmf_filename 
...
/**
 * @brief Get dimensions from an XML DataSet node
*/
std::vector<std::int64_t> get_dataset_shape(const pugi::xml_node& dataset_node);

/**
 * @brief Get the number of cells from an XML Topology node
 * 
 * @param topology_node 
 * @return std::int64_t 
 */
...
/**
 * @brief Get the VTK string identifier
*/
                   ...
                   const std::string& number_type, bool use_mpi_io)
{
  ...
  else
  {
    ...
    hdf5::write_dataset(h5_id, h5_path, x.data(), local_range, shape,
                            use_mpi_io, false);
