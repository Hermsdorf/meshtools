...
void TestPETScMatrix(ParallelMesh* pmesh)
{
    ...
    for(int iel = 0; iel < pmesh->get_n_elements(); ++iel)
    {
        ...
        {
            unsigned int gi = gindex[i];
            unsigned int li = gi - start;
            if(gi >= start && gi <  end)
            {
               for(int j =0; j < connsz; ++j) {
                   unsigned int gj = gindex[j]; 
                   if(gj >= start && gj <  end)
                        vdiag[li].insert(gj);
                    else
                        voff[li].insert(gj);
...
void TestPETScISVector(ParallelMesh* pmesh)
{
    ...
    Vec local;  // size n - nodes
    Vec global; // size n - eq
    IS  is_local;
    IS  is_global; 
    VecScatter scatter;
    ...
    int n_local  = pmesh->get_n_local_nodes();
    auto &gindex = pmesh->getLocal2Global();
    ...
    VecCreateSeq(PETSC_COMM_SELF,n_nodes,&local);
    VecCreateMPI(MeshTools::Comm(),n_local,PETSC_DETERMINE,&global);

    VecScatterCreate(global,is_global,local, is_local, &scatter);
    PetscScalar *values;
    PetscInt array_local_size;
    VecGetLocalSize(global, &array_local_size);
    VecGetArray(global,&values);

    for(int i = 0; i < array_local_size; ++i)
        values[i] = MeshTools::processor_id();

    VecRestoreArray(global,&values);
    
    VecScatterBegin(scatter,global,local,INSERT_VALUES,SCATTER_FORWARD);
    VecScatterEnd(scatter,global,local,INSERT_VALUES,SCATTER_FORWARD);

    VecView(local, PETSC_VIEWER_STDOUT_SELF);
