...
void assemble_transport(TransientImplicitSystem* system)
{
    ...
    for (int iel = 0; iel < n_elements; iel++)
    {
        ...
        for (int q = 0; q < qrule.n_points(); q++)
        {
            ...
            const double h    = std::sqrt(2.0*JxW);
            // CAU stabilization parameters
            const double ctau = cau_stab(u, u_old, grad_u, source_term, velocity, sigma, k, dt, h, dxi, deta, dzeta)*0.2;
...
int transport(int argc, char *argv[])
{
    ...
    if (processor_id == 0)
    {
        ...
        // irá ler a malha.
        mesh = new Mesh("/home/camata/git/meshtools/test/finite_element/msh/transport/test1.msh");
